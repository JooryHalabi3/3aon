...
function updatePageTitle() {
  if (userDepartment && userDepartment.name) {
    ...
    if (pageTitle) {
      pageTitle.textContent = `سجلات ${userDepartment.name}`;
    }
    
    if (pageSubtitle) {
      pageSubtitle.textContent = `تتبع الأنشطة والعمليات الخاصة بـ ${userDepartment.name}`;
    }
  }
}
...
function applyFilters() {
  ...
  if (activityType) currentFilters.activityType = activityType;
  if (searchInput) currentFilters.search = searchInput;
  
  // إضافة فلتر القسم
  if (userDepartment && userDepartment.id) {
    currentFilters.departmentId = userDepartment.id;
  }
...
async function loadLogs() {
  ...
  try {
    ...
    // إضافة فلتر القسم إلى المعاملات
    const params = new URLSearchParams({
      page: currentPage,
      limit: 10,
      ...currentFilters
    ...
    // إضافة فلتر القسم إذا كان متوفراً
    if (userDepartment && userDepartment.id) {
      params.append('departmentId', userDepartment.id);
    }
...
function updateStatsCards(stats) {
  ...
  const departmentUsersElement = document.getElementById('departmentUsers');

  if (totalLogsElement) {
    totalLogsElement.textContent = stats.totalLogs || 0;
  }
  
  if (todayLogsElement) {
    todayLogsElement.textContent = stats.todayLogs || 0;
  }
  
  if (departmentUsersElement) {
    departmentUsersElement.textContent = stats.departmentUsers || 0;
  }
}
