...
const transferComplaint = async (req, res) => {
  try {
    ...
    const [complaintCheck] = await pool.execute(
      'SELECT ComplaintID, DepartmentID, CurrentStatus FROM Complaints WHERE ComplaintID = ?',
      [complaintId]
    );
    ...
    const oldDepartmentId = complaintCheck[0].DepartmentID;
    ...
    // تحديث قسم الشكوى
    await pool.execute(
      'UPDATE Complaints SET DepartmentID = ? WHERE ComplaintID = ?',
    ...
    await pool.execute(
      ...
      [
        complaintId, 
        req.user ? req.user.employeeID : 1, 
        'تحويل الشكوى', 
        `تم تحويل الشكوى من قسم (ID: ${oldDepartmentId}) إلى قسم (ID: ${newDepartmentId})`, 
        complaintCheck[0].CurrentStatus, 
        complaintCheck[0].CurrentStatus
      ]
    );

    console.log(`✅ تم تحويل الشكوى ${complaintId} من قسم ${oldDepartmentId} إلى قسم ${newDepartmentId}`);
    ...
    res.json({
      ...
      data: {
        ...
        newDepartmentName: departmentCheck[0].DepartmentName
      }
    });
